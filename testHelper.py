...
def getCountrangepartition(ratingstablename, numberofpartitions, openconnection):
    # Hàm này dùng để đếm số lượng dòng (rows) trong bảng ratingstablename thuộc về từng partition range, dựa trên giá trị rating.
    # Mỗi partition sẽ tương ứng với một khoảng giá trị rating.
    """
...
def checkpartitioncount(cursor, expectedpartitions, prefix):
    # Kiểm tra xem số lượng bảng partition (theo prefix cho trước) trong cơ sở dữ liệu có đúng bằng số lượng mong đợi (expectedpartitions) không.
...
def totalrowsinallpartitions(cur, n, rangepartitiontableprefix, partitionstartindex):
    # Xác định xem sau khi phân mảnh có thừa hay thiếu so với bảng gốc không
    selects = []
...
def testrangeandrobinpartitioning(n, openconnection, rangepartitiontableprefix, partitionstartindex, ACTUAL_ROWS_IN_INPUT_FILE):
   # Kiểm tra tính đúng đắn của việc phân mảnh
    with openconnection.cursor() as cur:
        if not isinstance(n, int) or n < 0:
            # Test 1: Kiểm tra không có bảng phân mảnh nào được tạo ra vì n không hợp lệ
            checkpartitioncount(cur, 0, rangepartitiontableprefix)
        else:
            # Test 2: Kiểm tra đã tạo đúng số bảng partition (n bảng).
            checkpartitioncount(cur, n, rangepartitiontableprefix)

            # Test 3: Sau khi gộp tất cả các partition lại, tổng số dòng phải không nhỏ hơn số dòng gốc.
            ...
            # Test 4: Sau khi gộp các partition, tổng số dòng không được lớn hơn số dòng gốc.
            count = totalrowsinallpartitions(cur, n, rangepartitiontableprefix, partitionstartindex)
            if count > ACTUAL_ROWS_IN_INPUT_FILE: raise Exception(
                "Dijointness property of Partitioning failed. Excpected {0} rows after merging all tables, but found {1} rows".format(
                    ACTUAL_ROWS_IN_INPUT_FILE, count))

            # Test 5:  Sau khi gộp lại, tổng số dòng phải đúng bằng số dòng gốc.
            count = totalrowsinallpartitions(cur, n, rangepartitiontableprefix, partitionstartindex)
            if count != ACTUAL_ROWS_IN_INPUT_FILE: raise Exception(
...
def testrangerobininsert(expectedtablename, itemid, openconnection, rating, userid):
     # Hàm này giúp kiểm tra tính chính xác của thao tác insert vào partition/table, đảm bảo mỗi bản ghi chỉ xuất hiện đúng một lần.
    with openconnection.cursor() as cur:
        cur.execute(
...
def testEachRangePartition(ratingstablename, n, openconnection, rangepartitiontableprefix):
   # Kiểm tra xem mỗi partition theo kiểu range có đúng số dòng như mong đợi hay không.
...
def testEachRoundrobinPartition(ratingstablename, n, openconnection, roundrobinpartitiontableprefix):
  # Kiểm tra xem mỗi partition theo kiểu round robin có đúng số dòng như mong đợi hay không.
